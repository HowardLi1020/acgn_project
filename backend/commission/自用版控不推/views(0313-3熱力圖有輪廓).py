...
def generate_gradcam(image_path, target_vector=None):
    try:
        ...
        if hasattr(model.visual, 'transformer'):
            # ViT 模型使用倒數第二層 Transformer 塊
            block = model.visual.transformer.resblocks[-2]  # 選擇倒數第二層
            hooks.append(block.register_forward_hook(get_features))
            hooks.append(block.register_full_backward_hook(get_gradients))
        else:
            # 對於CNN模型，使用最後一個卷積層
            for name, module in reversed(list(model.visual.named_modules())):
                if isinstance(module, torch.nn.Conv2d):
                    hooks.append(module.register_forward_hook(get_features))
                    hooks.append(module.register_full_backward_hook(get_gradients))
                    break
        ...
        print(f"圖片相似度: {similarity.item():.4f}")
        ...
        if features is not None and gradients is not None:
            ...
            if hasattr(model.visual, 'transformer'):
                ...
                if len(features.shape) == 3:  # [B, N, C]
                    ...
                    weights = gradients.mean(dim=[0, 2])  # [B, N, C] -> [N]
                    cam = torch.matmul(features[0], weights)  # [N, C] x [C] -> [N]
                    cam = cam.reshape(h, w)  # 重塑為空間維度
                else:
                    print(f"未預期的特徵形狀: {features.shape}")
                    raise ValueError(f"無法處理形狀為 {features.shape} 的特徵")
            else:
                # CNN 模型
                weights = gradients.mean([0, 2, 3])  # 全局平均池化梯度
                cam = torch.zeros(features.shape[0], features.shape[2], features.shape[3], device=device)
            ...
            if cam.max() > cam.min():
                ...
                # 應用非線性轉換以增強對比度
                cam = np.power(cam, 0.7)  # 指數小於1會增強弱激活區域的可見性
            else:
                print("警告：生成的熱力圖全是相同值，重置為全0")
                cam = np.zeros((img.height, img.width))

            # 雙向色彩映射：藍色代表較不相似，紅色代表較相似
            heatmap = np.zeros((img.height, img.width, 3), dtype=np.uint8)
            
            # 紅色通道 - 高相似度
            heatmap[:, :, 0] = np.uint8(255 * cam)
            # 藍色通道 - 低相似度（反轉cam）
            heatmap[:, :, 2] = np.uint8(255 * (1 - cam))
...
def _generate_fallback_heatmap(image_path):
    try:
        ...
        # 邊緣區域為紅色 (高相似度)
        heatmap[edges > 0, 0] = 255
        # 非邊緣區域為藍色 (低相似度)
        heatmap[edges == 0, 2] = 255
...
def compare_images(uploaded_vector, uploaded_path):
    ...
    try:
        ...
        uploaded_vector_normalized = uploaded_vector / np.linalg.norm(uploaded_vector)
        
        # 比較每一張圖片
        for image in work_images:
            if not image.image_url:
                print(f"跳過無URL的圖片: ID={image.image_id}")
                continue
            ...
            if image_vector is None:
                print(f"無法計算圖片向量: {image_path}")
                continue
                
            # 計算相似度 (cos相似度)
            image_vector_normalized = image_vector / np.linalg.norm(image_vector)
            ...
            if similarity > best_similarity:
                ...
                best_match = image
                print(f"找到新的最佳匹配: {image.image_url}, 相似度: {similarity:.4f}")
        
        # 如果找到匹配的圖片
        if best_match:
            best_match_path = os.path.join(settings.MEDIA_ROOT, "commission", "workID_img", best_match.image_url)
            print(f"最終最佳匹配: {best_match_path}, 相似度: {best_similarity:.4f}")
            ...
            if similarity_percent > 90:
                ...
                similarity_description = "這兩張圖片相似度較低，僅顯示了資料庫中最接近的匹配項。"
            ...
            try:
                print("為上傳圖片生成熱力圖...")
                # 1. 首先為上傳的圖片生成熱力圖，高亮與最佳匹配相似的區域
                uploaded_gradcam_path = generate_gradcam(
                    uploaded_path, 
                    get_image_vector(best_match_path)  # 使用匹配圖片的向量作為目標
                )
                if uploaded_gradcam_path and os.path.exists(uploaded_gradcam_path):
                    gradcam_paths['uploaded'] = uploaded_gradcam_path
                    print(f"上傳圖片熱力圖已生成: {uploaded_gradcam_path}")
                else:
                    print("上傳圖片熱力圖生成失敗或路徑不存在")
                
                print("為最佳匹配圖片生成熱力圖...")
                # 2. 為最佳匹配的圖片生成熱力圖，高亮與上傳圖片相似的區域
                best_match_gradcam_path = generate_gradcam(
                    best_match_path, 
                    uploaded_vector  # 使用上傳圖片的向量作為目標
                )
                if best_match_gradcam_path and os.path.exists(best_match_gradcam_path):
                    gradcam_paths['best_match'] = best_match_gradcam_path
                    print(f"最佳匹配圖片熱力圖已生成: {best_match_gradcam_path}")
                else:
                    print("最佳匹配圖片熱力圖生成失敗或路徑不存在")
                    
            except Exception as e:
                print(f"生成熱力圖時出錯: {e}")
                print(traceback.format_exc())
            
            return best_match, similarity_percent, similarity_description, gradcam_paths
        else:
            print("未找到任何匹配的圖片")
...
def upload_and_compare(request):
    ...
    if request.method == 'POST' and request.FILES.get('image'):
        ...
        context["uploaded_image"] = file_name
        ...
        # 在資料庫內部比對
        try:
            best_match, similarity, similarity_description, gradcam_paths = compare_images(uploaded_vector, file_path)
            ...
            if best_match:
                ...
                if 'best_match' in gradcam_paths:
                    # 從絕對路徑轉換為相對路徑
