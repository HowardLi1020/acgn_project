...
def compare_images(uploaded_vector, uploaded_path):
    ...
    Returns:
        一個元組 (最相似圖片信息, 相似度百分比, 相似度描述, 相似圖片列表)
    """
    try:
        # 取得資料庫中所有圖片
        ...
        # 取前3個最相似的結果（如果有那麼多）
        top_matches = all_similarities[:min(3, len(all_similarities))]
        ...
        if top_matches:
            ...
            return best_match, similarity_percent, similarity_description, top_matches
        else:
            print("未找到任何匹配的圖片")
...
def upload_and_compare(request):
    ...
    if request.method == 'POST' and request.FILES.get('image'):
        ...
        # 在資料庫內部比對
        try:
            best_match, similarity, similarity_description, top_matches = compare_images(uploaded_vector, file_path)
                ...
                if len(top_matches) > 1:
                    ...
                    for i, (image, sim, path) in enumerate(top_matches[1:], 2):  # 從第二個開始，索引從2開始
                        similar_images.append({
                            'image_url': os.path.join(settings.MEDIA_URL, "commission", "workID_img", image.image_url),
