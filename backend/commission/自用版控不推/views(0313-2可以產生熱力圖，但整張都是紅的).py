...
def generate_gradcam(image_path, target_vector=None):
    try:
        ...
        # 對於 ViT 模型，我們獲取視覺編碼器的最後一層輸出
        # 注意: 根據實際模型結構可能需要調整
        if hasattr(model.visual, 'transformer'):
            # ViT 架構
            hook = model.visual.transformer.resblocks[-1].register_forward_hook(get_features)
            hooks.append(hook)
        else:
            # 其他架構
            for name, module in reversed(list(model.visual.named_modules())):
                if isinstance(module, torch.nn.Conv2d):
                    hook = module.register_forward_hook(get_features)
                    hooks.append(hook)
                    break
        ...
        with torch.no_grad():
            if target_vector is None:
                # 如果沒有提供目標向量，使用當前圖片的特徵
                target_vector = model.encode_image(input_tensor).flatten()
                # 確保是單位向量
                target_vector = target_vector / torch.norm(target_vector)
            elif isinstance(target_vector, np.ndarray):
                # 如果是 numpy 數組，轉換為 tensor
                target_vector = torch.tensor(target_vector, device=device)
                # 確保是單位向量
                target_vector = target_vector / torch.norm(target_vector)
        ...
        # 計算相似度（點積）
        similarity = torch.sum(output.flatten() * target_vector)
        ...
        # 如果是相同圖片，使用特殊處理
        is_same_image = similarity.item() > 0.95  # 相似度閾值
        
        if is_same_image:
            print("檢測到相同或非常相似的圖片，使用特殊熱力圖生成方法")
            # 對於相同圖片，我們生成一個全高關注的熱力圖
            heatmap = np.ones((img.height, img.width)) * 0.8  # 高關注度
            
            # 添加一些隨機變化使熱力圖更自然
            heatmap += np.random.rand(img.height, img.width) * 0.2
            ...
            if features is not None:
                ...
                # 根據 features 的形狀選擇處理方法
                if len(features.shape) == 4:  # CNN 特徵 [batch, channels, height, width]
                    # 計算特徵的重要性權重
                ...
                elif len(features.shape) == 3:  # Transformer 特徵 [batch, tokens, channels]
                    # 使用注意力機制
                    # 提取 CLS token 的梯度作為權重
                    weights = gradients[0, 0, :]  # CLS token 梯度
                    cam = torch.matmul(features[0], weights)
                    ...
                    # 回退到輸入梯度
                    cam = torch.sum(torch.abs(gradients[0]), dim=0)
                ...
                # 如果特徵提取失敗，使用輸入梯度
                gradients = input_tensor.grad.abs().cpu().numpy()
                heatmap = np.sum(gradients[0], axis=0)  # 沿通道維度求和
        ...
        superimposed_img = heatmap_colored * 0.5 + img_array * 0.5  # 增加熱力圖的權重
        ...
        # 打印生成的熱力圖路徑，用於調試
        print(f"已生成熱力圖: {gradcam_path}, 相似度: {similarity.item():.4f}")
        
        return gradcam_path
        
    except Exception as e:
        print(f"生成熱力圖時出錯: {e}")
        print(f"錯誤位置: {traceback.format_exc()}")
        ...
        try:
            ...
            # 創建一個熱力圖（紅色為主）
            heat_array = np.ones((img.height, img.width, 3)) * np.array([255, 50, 50])  # 紅色基調
            heat_array += np.random.rand(img.height, img.width, 3) * 50 - 25  # 添加些微變化
            heat_array = np.clip(heat_array, 0, 255).astype(np.uint8)
            
            # 圖像與熱力圖疊加
            superimposed_img = heat_array * 0.4 + img_array * 0.6
            superimposed_img = np.clip(superimposed_img, 0, 255).astype(np.uint8)
            ...
            print(f"已生成備用熱力圖: {fallback_path}")
            
            return fallback_path
            
        except Exception as e2:
            print(f"生成備用熱力圖時也出錯: {e2}")
            return image_path  # 如果所有嘗試都失敗，返回原始圖像路徑
...
def compare_images(uploaded_vector, uploaded_path):
    best_match = None
    best_similarity = -1  # 初始化為一個不可能的值
    gradcam_paths = {}
    
    try:
        # 使用 DbWorkImages 獲取所有作品圖片
        work_images = DbWorkImages.objects.all()
        
        for db_image in work_images:
            # 構建完整的圖片路徑
            ...
            try:
                # 計算圖片向量
                db_vector = get_image_vector(image_path)
                
                # 計算餘弦相似度
                similarity = np.dot(uploaded_vector, db_vector) / (
                    np.linalg.norm(uploaded_vector) * np.linalg.norm(db_vector)
                )
                ...
                print(f"處理圖片時出錯 {image_url}: {e}")
                continue
        
        # 如果找到了最佳匹配
        if best_match:
            best_match_path = os.path.join(settings.MEDIA_ROOT, "commission", "workID_img", best_match.image_url)
            
            try:
                # 為上傳的圖片生成熱力圖
                print(f"為上傳圖片生成熱力圖: {uploaded_path}")
                uploaded_gradcam_path = generate_gradcam(uploaded_path, target_vector=None)
                if uploaded_gradcam_path and os.path.exists(uploaded_gradcam_path):
                    gradcam_paths['uploaded'] = uploaded_gradcam_path
                    print(f"上傳圖片熱力圖生成成功: {uploaded_gradcam_path}")
                
                # 為最佳匹配圖片生成熱力圖
                print(f"為最佳匹配圖片生成熱力圖: {best_match_path}")
                best_match_gradcam_path = generate_gradcam(best_match_path, target_vector=uploaded_vector)
                if best_match_gradcam_path and os.path.exists(best_match_gradcam_path):
                    gradcam_paths['best_match'] = best_match_gradcam_path
                    print(f"最佳匹配熱力圖生成成功: {best_match_gradcam_path}")
                
                # 檢查是否是相同或幾乎相同的圖片
                if best_similarity > 0.95:
                    print(f"檢測到非常相似的圖片: {best_similarity:.4f}")
            except Exception as e:
                print(f"生成熱力圖時出錯: {e}")
                print(f"錯誤位置: {traceback.format_exc()}")
        
        return best_match, best_similarity, gradcam_paths
...
def upload_and_compare(request):
    ...
    if request.method == 'POST' and request.FILES.get('image'):
        ...
        # 在資料庫內部比對
        try:
            best_match, similarity, gradcam_paths = compare_images(uploaded_vector, file_path)
            
            # 判斷是否為高度相似
            is_very_similar = similarity > 0.95
            ...
            # 添加相似度描述
            if is_very_similar:
                context["similarity_description"] = f"極高相似度 ({similarity_percentage}%)，可能是相同圖片或高度修改版本"
            elif similarity > 0.85:
                context["similarity_description"] = f"高度相似 ({similarity_percentage}%)，大部分特徵相符"
            elif similarity > 0.70:
                context["similarity_description"] = f"中度相似 ({similarity_percentage}%)，有顯著相似特徵"
            else:
                context["similarity_description"] = f"低度相似 ({similarity_percentage}%)，僅有少量相似特徵"
            ...
            if best_match:
                ...
                if gradcam_paths:
                    # 安全地處理熱力圖 URL
                    if 'uploaded' in gradcam_paths and os.path.exists(gradcam_paths['uploaded']):
                        # 從絕對路徑獲取相對路徑
                        file_name = os.path.basename(gradcam_paths['uploaded'])
                        context["uploaded_gradcam_url"] = f"{settings.MEDIA_URL}uploads/{file_name}"
                        print(f"上傳圖片熱力圖 URL: {context['uploaded_gradcam_url']}")
                    
                    if 'best_match' in gradcam_paths and os.path.exists(gradcam_paths['best_match']):
                        # 從絕對路徑獲取相對路徑
                        file_name = os.path.basename(gradcam_paths['best_match'])
                        context["best_match_gradcam_url"] = f"{settings.MEDIA_URL}uploads/{file_name}"
                        print(f"匹配圖片熱力圖 URL: {context['best_match_gradcam_url']}")
                
                # 如果需要顯示作品信息，添加相關作品信息
                if hasattr(best_match, 'work') and best_match.work:
