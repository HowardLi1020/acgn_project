...
# 3. 比對圖片相似度
def compare_images(uploaded_vector):
    # 使用 DbWorkImages 替代 ImageModel
    ...
    for img in work_images:
        ...
        # 計算資料庫圖片的向量
        db_vector = get_db_image_vector(img.image_url)
        if db_vector is None:
            continue
            
        # 計算相似度
        similarity = np.dot(uploaded_vector, db_vector) / (np.linalg.norm(uploaded_vector) * np.linalg.norm(db_vector))
...
def upload_and_compare(request):
    ...
    if request.method == "POST" and request.FILES.get("image"):
        ...
        # 在資料庫內部比對
        best_match, similarity = compare_images(uploaded_vector)
        ...
        if best_match:
            ...
            # 如果需要顯示作品信息，可以添加相關作品信息
            if best_match.work:
                context["work_info"] = best_match.work

        return render(request, "commission/result.html", context)

    print("DEBUG: 嘗試渲染 commission/upload.html")
    try:
        return render(request, "commission/upload.html", context)
    except Exception as e:
        print(f"錯誤: {e}")
        # 如果第一種方式失敗，嘗試直接使用絕對路徑
        try:
            from django.template.loader import get_template
            # 嘗試直接獲取模板
            template = get_template("commission/upload.html")
            return HttpResponse(template.render(context, request))
        except Exception as e2:
            print(f"第二次嘗試錯誤: {e2}")
            # 返回簡單的 HttpResponse 作為備用
            html = """
            <html><body>
            <h1>圖片上傳比對</h1>
            <form action="/commission/upload/" method="post" enctype="multipart/form-data">
                <input type="hidden" name="csrfmiddlewaretoken" value="">
                <input type="file" name="image">
                <button type="submit">上傳並比對</button>
            </form>
            </body></html>
            """
