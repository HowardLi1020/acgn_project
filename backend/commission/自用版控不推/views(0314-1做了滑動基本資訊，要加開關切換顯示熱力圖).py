            ...
            try:
                ...
                if rollout is not None:
                    ...
                    if best_match_attention_path and os.path.exists(best_match_attention_path):
                        attention_paths['best_match'] = best_match_attention_path
        ...
        try:
            ...
            if best_match:
                ...
                if 'uploaded' in attention_paths:
                    # 從絕對路徑轉換為URL
                    file_name = os.path.basename(attention_paths['uploaded'])
                    context["uploaded_attention_url"] = f"{settings.MEDIA_URL}uploads/{file_name}"
                    print(f"上傳圖片的注意力圖 URL: {context['uploaded_attention_url']}")
                    # 檢查文件是否存在
                    full_path = os.path.join(settings.MEDIA_ROOT, 'uploads', file_name)
                ...
                if 'best_match' in attention_paths:
                    # 從絕對路徑轉換為URL
                    file_name = os.path.basename(attention_paths['best_match'])
                    context["best_match_attention_url"] = f"{settings.MEDIA_URL}uploads/{file_name}"
                ...
                if len(top_matches) > 1:
                    similar_images = []
                    for i, (image, sim, path) in enumerate(top_matches[1:], 2):  # 從第二個開始，索引從2開始
                        similar_image_url = os.path.join(settings.MEDIA_URL, "commission", "workID_img", image.image_url)
                        similar_images.append({
                            'image_url': similar_image_url,
                            'similarity': round(sim * 100, 2),
                            'rank': i
                        })
                        print(f"添加相似圖片 #{i}: {similar_image_url}, 相似度: {round(sim * 100, 2)}%")
                    
                    context["similar_images"] = similar_images
                    print(f"添加額外的相似圖片: {len(similar_images)} 張")
                else:
                    print("沒有額外的相似圖片可顯示")
            else:
                print("未找到匹配的圖片")
